...
public class StartServProc extends ProcWithLog {

	private static Logger logger = LoggerFactory.getLogger(StartServProc.class);
	...
	public void executeWithLog() throws Exception {
		...
		logger.debug("启动服务器:  " + JsonUtil.toJson(procInput));
		...
		String startCmd = String.valueOf(params.get("serverStartCmd"));

		AgentCmd agentCmd = new AgentCmd(procInput.getFlowId(), agentIp, AgentCmdType.START_SERVER);

		agentCmd.addParam("jmxPort", jmxPort);

		agentCmd.addParam("shell", startCmd);
